...
const detailWithRelatedBlogs = async (id, initialData) => {
  try {
    const { data } = await fetchApi.get(`${mainRoute}/${id}/detail`, initialData);
    return data.body;
  } catch (error) {
    return { error: true, message: getAxiosError(error) };
  }
...
const servicesApi = { list, listActive, detail, detailWithRelatedBlogs, create, update, remove };
